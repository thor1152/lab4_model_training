...
def train_model_v2(
    ...
    metrics_path: str = "eval/iris_metrics.json",
    metadata_path: str = "models/iris_metadata.json",
    ) -> float:
    ...
    print(f"[ml_pipeline.v2_model] Model accuracy: {acc:.4f}")

    model_version = datetime.now().strftime("%Y%m%d_%H%M%S")

    os.makedirs(os.path.dirname(model_path), exist_ok=True)
    joblib.dump(pipeline, model_path)
    print(f"[ml_pipeline.V2_model] Saved model to {model_path}")

    metrics = {"accuracy": float(acc)}
    with open(metrics_path, "w") as f:
        json.dump(metrics, f, indent=2)
    print(f"[ml_pipeline.V2_model] Saved metrics to {metrics_path}")
    ...
    print(f"[ml_pipeline.v2_model] Saved metadata to {metadata_path}")
...
def promote_model_v2(
    ...
    s3_prefix = f"iris-models/{model_version}"
    s3 = boto3.client("s3")
    ...
    print(f"[ml_pipeline.v2_model] Promoted model to s3://{bucket_name}/{s3_prefix}/")
